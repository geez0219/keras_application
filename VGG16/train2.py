...
tb_cb = tf.keras.callbacks.TensorBoard(log_dir="tensorboard/batch_dropout")
cp_cb = tf.keras.callbacks.ModelCheckpoint(filepath="checkpoint.h5")

model.fit(x_train, y_train, batch_size=128, initial_epoch=100, epochs=200, validation_split=0.1, callbacks=[tb_cb, cp_cb])
